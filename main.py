...
# --- BẢNG NHẬP LIỆU (MODAL) CHO SPAM NHANH (DM hoặc SERVER) ---
class ModalSpamNhanh(discord.ui.Modal, title="⚡ CẤU HÌNH CHIẾN DỊCH SPAM"):
  ...
  def __init__(self, mode_type: str, target_member=None):
    super().__init__()
    self.mode_type = mode_type  # 'server' hoặc 'dm'
...
# --- BẢNG NHẬP LIỆU (MODAL) CHO TREO GIỜ ---
class ModalTreoGio(discord.ui.Modal, title="🛡️ CẤU HÌNH HỆ THỐNG TREO NGẦM"):
  ...
  def __init__(self, treo_type: str, target_member=None):
    super().__init__()
    self.treo_type = treo_type  # 'treo_server' hoặc 'treo_dm'
    self.target_member = target_member
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    if self.treo_type == "treo_server":
      ...
      await interaction.followup.send(
          ...
          f" Thời gian: **{hours} tiếng**\n- Tần suất: Cứ **{interval}** giây"
          " gửi 1 tin.",
          ephemeral=True,
      )
...
# --- MENU BƯỚC 2 CHO PHẦN TREO GIỜ (CHỌN TREO DM HAY SERVER) ---
...
class ViewMenuChinh(discord.ui.View):
  ...
  async def select_main(
    ...
    if choice == "spam_server":
      ...
      # Chuyển sang menu chọn kiểu treo (DM hay Server)
