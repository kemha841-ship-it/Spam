...
# Link ảnh Itachi Sharingan chuẩn nét, không bao giờ mất ảnh
ITACHI_IMAGE_URL = "https://images.hdqwalls.com/wallpapers/uchiha-itachi-minimal-art-4k-uh.jpg"
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    # 2. Treo DM
      ...
      if not self.target_member:
        ...
        return
      await interaction.followup.send(
          f"🛡️ **Đã bật Treo DM mục tiêu {self.target_member.name} dalam"
          f" {hours} tiếng!**\n- Gõ `!stop` để dừng bất cứ lúc nào.",
          ephemeral=True,
      )
