...
# Mặc định sẵn ảnh Itachi ngầu lòi do sếp cung cấp
DEFAULT_IMAGE_URL = (
...
# Hàm tạo Embed viền 7 màu lấp lánh đi kèm ảnh Itachi mặc định
def create_rainbow_embed(content: str):
  ...
  embed.set_footer(
      text="✨ Itachi Sharingan - Vòng lặp 7 màu độc quyền 2026 ✨"
  )
  return embed
...
# --- MODAL 1: NHẬP CHỮ ĐỂ SPAM SERVER (0.001S SONG SONG) ---
class ModalSpamServer(discord.ui.Modal, title="⚡ SPAM KÊNH SERVER (ITACHI 0.001S)"):
...
# --- MODAL 2: NHẬP CHỮ ĐỂ SPAM DM (0.001S) ---
class ModalSpamDM(discord.ui.Modal, title="🎯 SPAM DM (ITACHI 0.001S)"):
...
# --- MODAL 3: NHẬP CHỮ ĐỂ TREO NGẦM ---
class ModalTreoMay(discord.ui.Modal, title="🛡️ TREO NGẦM (ITACHI 0.001S)"):
...
# --- VIEW CHỌN CHẾ ĐỘ (HIỆN THẲNG KHUNG NHẬP VĂN BẢN) ---
class ViewChonCheDo(discord.ui.View):
  ...
  @discord.ui.button(
      label="1. Spam Server (Ảnh Itachi Mặc Định)",
  ...
  @discord.ui.button(
      label="2. Spam DM (Ảnh Itachi Mặc Định)",
  ...
  @discord.ui.button(
      label="3. Treo Máy Ngầm (Ảnh Itachi Mặc Định)",
...
@bot.tree.command(
    name="spam", description="Bảng điều khiển Itachi siêu tốc (Tự nhập chữ)"
)
...
async def spam(
  ...
  await interaction.response.send_message(
      "🔥 **[HỆ THỐNG ĐIỀU KHIỂN ITACHI TỐI THƯỢNG]**\nSếp chọn chế độ bên"
      " dưới để nhập nội dung:",
      view=view,
      ephemeral=True,
  )
