...
@bot.tree.command(name="spam", description="Gửi tin nhắn riêng hối thúc học bài chuyên nghiệp")
...
async def spam(interaction: discord.Interaction, target: str, noidung: str, solan: int, delay: int):
    if solan > 500:
        await interaction.response.send_message("❌ Tối đa chỉ gửi được 500 tin nhắn thôi sếp ơi!", ephemeral=True)
        return
    ...
    # Phản hồi ngay lập tức để Discord không báo lỗi "Ứng dụng không phản hồi"
    ...
    if not target_user:
        await interaction.followup.send(f"❌ Không tìm thấy user nào có tên hoặc ID là `{target}`!", ephemeral=True)
        return

    # Thông báo bắt đầu chiến dịch
    await interaction.followup.send(f"🚀 Đã bắt đầu oanh tạc hòm thư riêng của **{target_user.name}** ({solan} tin)...", ephemeral=True)
    ...
    for i in range(1, solan + 1):
        try:
            ...
            await interaction.followup.send(f"⚠️ Dừng lại do lỗi (có thể do nó chặn tin nhắn người lạ): `{e}`", ephemeral=True)
            break
        
        if i < solan:
            await asyncio.sleep(delay)

    await interaction.followup.send(f"✅ Đã gửi thành công **{success}/{solan}** tin nhắn riêng cho **{target_user.name}**!", ephemeral=True)

@bot.event
async def on_ready():
    try:
        # Đồng bộ lệnh slash lên Discord cực kỳ mượt mà
